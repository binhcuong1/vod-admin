...
(() => {
  ...
  const ACTOR_SEARCH = (kw) =>
    `${API_BASE}/api/actors/search?keyword=${encodeURIComponent(kw || "")}`;
      ...
      .map((m) => {
        ...
        <tr data-id="${m.Film_id}">
            ...
            <div class="d-flex align-items-center">
              ${
                m.poster_main
                  ? `<img src="${esc(
                      m.poster_main
                    )}" style="width:48px;height:72px;object-fit:cover" class="mr-2 rounded">`
                  : ""
              }
              <div>
          ...
          <td class="text-center">
            <button class="btn btn-sm btn-info btn-view-mv" data-id="${
              m.Film_id
            }">Xem</button>
            <button class="btn btn-sm btn-primary btn-edit-mv" data-id="${
              m.Film_id
            }">Sửa</button>
            <button class="btn btn-sm btn-danger btn-del-mv" data-id="${
              m.Film_id
            }">Xóa</button>
          </td>
        </tr>
  ...
  function fillForm(m = {}) {
    ...
    if (Array.isArray(m.genre_ids)) {
      if (typeof setCheckedGenreIds === "function") {
        ...
        if (typeof selectValues === "function")
          selectValues("#Genre_ids", m.genre_ids);
      }
    }
    ...
    if (Array.isArray(m.cast_ids)) {
      if (typeof setCheckedActorIds === "function") {
        ...
        if (typeof selectValues === "function")
          selectValues("#Cast_ids", m.cast_ids);
      }
    }
  }
  ...
  async function onSubmit(e) {
    e.preventDefault();
    if (overviewQuill) {
    const delta = overviewQuill.getContents();      
    qs("#Overview").value = JSON.stringify(delta);  
}
      ...
      .map(
        (s, idx) => `
          <div class="card mb-2">
            ...
            <div id="seasonCollapse-${s.id}" class="collapse ${
          idx === 0 ? "show" : ""
        }">
  ...
  function renderEpisodesForSeason(seasonId, episodes) {
    ...
    const rowsHtml = (episodes || [])
      ...
      <tr>
        ...
        <td class="text-center align-middle">
          ...
          <button type="button" class="btn btn-xs btn-info btn-edit-ep" data-ep="${
            e.id
          }">
            <i class="fas fa-edit"></i>
          </button>
          <button type="button" class="btn btn-xs btn-danger btn-del-ep" data-ep="${
            e.id
          }">
            <i class="fas fa-trash"></i>
          </button>
    ...
    tb.querySelectorAll(".btn-edit-ep").forEach((btn) => {
      btn.onclick = (ev) => {
        ...
        const curDurText = durCell
          ? durCell.textContent.trim().replace("p", "")
          : "";
  ...
  async function openEpSources(ep) {
    // ep: {id, number, title}
    // hiển thị tiêu đề
    ...
    const dict = Object.fromEntries(
      rows.map((x) => [String(x.Resolution_id), x.Source_url || ""])
    );

    // render bảng
    const tb = qs("#epSourceBody");
    if (tb) {
      tb.innerHTML = RESOLUTIONS.map(
        (r) => `
      <tr>
  ...
  async function saveEpSources(epId) {
    ...
    const sources = inputs
      .map((i) => ({
        resolution_id: Number(i.dataset.res),
        source_url: i.value.trim(),
      }))
  ...
  async function bindLookups() {
    try {
      ...
      if (!sel) {
        ...
        const countries = (Array.isArray(raw) ? raw : [])
          .map((c) => ({
            ...
            name:
              c.name ?? c.Country_name ?? c.country_name ?? c.label ?? c.text,
          }))
      ...
      if (boxList && genres.length) {
        ...
        window.GenreDict = Object.fromEntries(
          genres.map((g) => [String(g.id), g.name])
        );

        boxList.innerHTML = genres
          .map(
            (g) => `
        <div class="form-check">
          <input class="form-check-input genre-cb" type="checkbox" id="g_${
            g.id
          }" value="${g.id}">
      ...
      if (actList && typeof loadActors === "function") {
        ...
        window.ActorDict = Object.fromEntries(
          actors.map((a) => [String(a.id), a.name])
        );
        ...
        const renderActorList = (rows) => {
          ...
          actList.innerHTML = (rows || [])
            .map(
              (a) => `
      <div class="form-check">
        <input class="form-check-input actor-cb" type="checkbox" id="a_${
          a.id
        }" value="${a.id}" ${cur.has(String(a.id)) ? "checked" : ""}>
    ...
    `
          ...
          try {
            ...
            const rows = (Array.isArray(found) ? found : [])
              .map((a) => ({
                id: a.id ?? a.Actor_id ?? a.actor_id ?? a.value,
                name:
                  a.name ?? a.Actor_name ?? a.actor_name ?? a.label ?? a.text,
              }))
              .filter((x) => x.id != null && x.name);

            // Cập nhật dict để chips hiển thị tên
            rows.forEach((a) => {
              window.ActorDict[String(a.id)] = a.name;
            });
        ...
        if (actSearch)
          actSearch.oninput = (e) => doSearch(e.target.value.trim());
        if (actClear)
          actClear.onclick = () => {
  ...
  const desc = qs("#Overview")?.value;

  // Khởi tạo Quill một lần
  if (!overviewQuill) {
    overviewQuill = new Quill("#OverviewEditor", {
      theme: "snow",
      placeholder: "Nhập mô tả phim...",
    });
  }

  //  Delay 1 tick để DOM modal ổn định
  setTimeout(() => {
    if (desc) {
      try {
        overviewQuill.setContents(JSON.parse(desc));
      } catch {
        overviewQuill.setText(desc);
      }
    } else {
      overviewQuill.setContents([]);
    }
  }, 50);   
});
}
  ...
  function renderDetail(d) {
    ...
    //Delta json -> htlm
    const descEl = qs("#mvDtlDesc");

if (descEl) {
    let raw = info?.description || "";
    if (!raw) {
        descEl.innerHTML = "<span class='text-muted'>Không có mô tả</span>";
    } else {
        try {
            const delta = JSON.parse(raw); // parse Delta
            const converter = new QuillDeltaToHtmlConverter(delta.ops || [], {});
            const html = converter.convert(); // chuyển sang HTML
    ...
    if (seriesWrap && seasonsBox) {
      ...
      const showSeries =
        !!(film && film.is_series) && seasons && seasons.length;
  ...
  function getSelectedGenreIds() {
    return Array.from(document.querySelectorAll(".genre-cb:checked")).map(
      (cb) => Number(cb.value)
    );
  }
  ...
  function getSelectedActorIds() {
    return Array.from(document.querySelectorAll(".actor-cb:checked")).map(
      (cb) => Number(cb.value)
    );
  }
  ...
  function renderActorChips(ids) {
    ...
    box.innerHTML = ids
      .map(
        (id) =>
          `<span class="badge badge-info mr-1 mb-1">${esc(
            dict[String(id)] ?? id
          )}</span>`
      )
      .join("");
  ...
  function setVal(sel, v) {
    const el = qs(sel);
    if (!el) {
      console.warn("[fillForm] missing", sel);
      return;
    }
    el.value = v ?? "";
  }
